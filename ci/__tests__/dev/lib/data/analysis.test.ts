...
const basePath = join(__dirname, '../../../../__mocks__/dev/data');
...
describe('Testing getAnalysis__.', () => second.forEach(({ label, input, output }) => {
    test(label, () => {
...
describe('Testing getCounters__.', () => third.forEach(({ label, input, output }) => {
    test(label, () => {
